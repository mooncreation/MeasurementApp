...
class MeasureViewController: UIViewController {
    ...
    private func layoutViewController() {
        ...
        do {
            ...
            copy.addTarget(self, action: #selector(MeasureViewController.copyAction(_:)), for: .touchUpInside)
            
            let tap = UITapGestureRecognizer(target: self, action: #selector(MeasureViewController.changeMeasureUnitAction(_:)))
        ...
        do {
            ...
            finishButton.addTarget(self, action: #selector(MeasureViewController.finishAreaAction(_:)), for: .touchUpInside)
            placeButton.addTarget(self, action: #selector(MeasureViewController.placeAction(_:)), for: .touchUpInside)
            placeButton.frame = CGRect(x: (width - 80)/2, y: (height - 20 - 80), width: 80, height: 80)
            finishButton.center = placeButton.center
        }
        do {
            view.addSubview(cancleButton)
            cancleButton.addTarget(self, action: #selector(MeasureViewController.deleteAction(_:)), for: .touchUpInside)
            cancleButton.frame = CGRect(x: 40, y: placeButton.frame.origin.y + 10, width: 60, height: 60)
        }
        do {
           // view.addSubview(menuButtonSet)
            menuButton.more.addTarget(self, action: #selector(MeasureViewController.showMenuAction(_:)), for: .touchUpInside)
            menuButton.setting.addTarget(self, action: #selector(MeasureViewController.moreAction(_:)), for: .touchUpInside)
            menuButton.reset.addTarget(self, action: #selector(MeasureViewController.restartAction(_:)), for: .touchUpInside)
            menuButton.measurement.addTarget(self, action: #selector(MeasureViewController.changeMeasureMode(_:)), for: .touchUpInside)
            menuButton.save.addTarget(self, action: #selector(MeasureViewController.saveImage(_:)), for: .touchUpInside)
...
// MARK: - UI
fileprivate extension MeasureViewController {
